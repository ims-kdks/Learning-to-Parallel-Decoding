...
accept_thres=0.9
accelerate launch eval.py --model dream \
    --model_args pretrained=${model},max_new_tokens=${gen_length},diffusion_steps=${gen_length},add_bos_token=true,alg=entropy,show_speed=True,block_length=${block_length},method=${method},accept_thres=${accept_thres} \
    --tasks ${task} \
    --batch_size 1 \
    --confirm_run_unsafe_code \
